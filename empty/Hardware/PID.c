...
void YawPID_Control(float target, float current_angle, int16_t base_speed, float gyroZ)
{
    ...
    /* 6. 最终占空比限幅 */
    tgt_l = (tgt_l > MOTOR_DUTY_MAX) ? MOTOR_DUTY_MAX : ((tgt_l < MOTOR_DUTY_MIN) ? MOTOR_DUTY_MIN : tgt_l);
    tgt_r = (tgt_r > MOTOR_DUTY_MAX) ? MOTOR_DUTY_MAX : ((tgt_r < MOTOR_DUTY_MIN) ? MOTOR_DUTY_MIN : tgt_r);
...
void Control(void)
{
    ...
    bias1 = HD_DIF();
    uint8_t Speed_Middle = 20;
 
    TargetA = Speed_Middle - bias1;
    TargetB = Speed_Middle + bias1;
 
    uint16_t Motor_Left  = PWM_Limit(PID_A(TargetA), 30, 1);
    uint16_t Motor_Right = PWM_Limit(PID_B(TargetB), 30, 1);
    Motor_SetSpeed(Motor_Left, Motor_Right);

    // char buf[16];

    
    // IntToStr((int)TargetA, buf);OLED_ShowString(0, 48, (uint8_t*)buf);
    // IntToStr((int)Motor_Left, buf);OLED_ShowString(12, 48, (uint8_t*)buf); 



    // IntToStr((int)TargetB, buf);OLED_ShowString(24, 48, (uint8_t*)buf);    
    // IntToStr((int)Motor_Right, buf);OLED_ShowString(36, 48, (uint8_t*)buf);
