...
//     HD[0] = (DL_GPIO_readPins(HD1_PORT, HD1_PIN_2_PIN) == 0) ? 1 : 0;
//     HD[1] = (DL_GPIO_readPins(HD2_PORT, HD2_PIN_3_PIN) == 0) ? 1 : 0;
//     HD[2] = (DL_GPIO_readPins(HD3_PORT, HD3_PIN_4_PIN) == 0) ? 1 : 0;
//     HD[3] = (DL_GPIO_readPins(HD4_PORT, HD4_PIN_5_PIN) == 0) ? 1 : 0;
// }
...
//     int r2 = HD[3];
//     int sum = l2 + l1 + r1 + r2;
//     if (sum == 0) return 9999.0f;      // 完全白色，返回极大值
//     // 加权平均：左边为负，右边为正
...
//         return 4;
//     }
//     else if (!HD[0] && !HD[1] && !HD[2] && !HD[3]) {  // 全白，可能离开黑线进入白底
//         return 0;
//         HD_Read();
//         delay_ms(5);
...
// {
//     char buf[16];
//         IntToStr(HD[0], buf);  OLED_ShowString(0, 0, (uint8_t*)buf);
//         IntToStr(HD[1], buf);  OLED_ShowString(12, 0, (uint8_t*)buf);
//         IntToStr(HD[2], buf);  OLED_ShowString(24, 0, (uint8_t*)buf);
//         IntToStr(HD[3], buf);  OLED_ShowString(36, 0, (uint8_t*)buf);
// }
